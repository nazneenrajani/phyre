...
int addBodiesToWorld(b2WorldWithData& world,
                      const std::vector<::scene::Body>& pThriftBodies,
                      const Box2dData::ObjectType object_type, bool do_print, int start) {
  size_t i;
  for (i = start; i < pThriftBodies.size(); ++i) {
    ...
    float x = body->GetWorldCenter().x, y = body->GetWorldCenter().y;
    if (do_print) {
      std::cout << "{\"id\": " << i << ", \"object_type\": ";
      if (object_type == Box2dData::USER) {
          std::cout << "\"user_circle\"";
          std::cout << ", \"x\": " << body->GetWorldCenter().x << ", \"y\": " << body->GetWorldCenter().y << "}], \"steps\": [";

      } else {
          if ((fabs(x + 0.41666) < 0.01 && fabs(y - 43.0833) < 0.01) ||
              (fabs(x - 21.3333) < 0.01 && fabs(y + 0.41666) < 0.01) ||
              (fabs(x - 21.3333) < 0.01 && fabs(y - 43.0833) < 0.01) ||
              (fabs(y - 21.3333) < 0.01 && fabs(x - 43.0833) < 0.01))
          {
              std::cout << "\"boundary\"";
          } else if (thriftBody.shapes.size() == 3) {
              std::cout << "\"jar\"";
          } else if (thriftBody.shapes[0].circle.radius) {
              std::cout << "\"circle\"";
          } else {
              std::cout << "\"bar\"";
          }
          std::cout << ", \"x\": " << body->GetWorldCenter().x << ", \"y\": " << body->GetWorldCenter().y << "}, ";
...
std::unique_ptr<b2WorldWithData> convertSceneToBox2dWorld(
    const ::scene::Scene& scene, bool do_print) {
  const b2Vec2 gravity(0.0f, DEFAULT_GRAVITY);
  std::unique_ptr<b2WorldWithData> world(new b2WorldWithData(gravity));
  int start = addBodiesToWorld(*world, scene.bodies, Box2dData::GENERAL, do_print, 0);
  addBodiesToWorld(*world, scene.user_input_bodies, Box2dData::USER, do_print, 0);
  return world;
}
...
std::unique_ptr<b2WorldWithData> convertSceneToBox2dWorld_with_bounding_boxes(
  ...
  addBodiesToWorld(*world, bounding_boxes, Box2dData::BOUNDING_BOX, false, 0);
  return world;
}
