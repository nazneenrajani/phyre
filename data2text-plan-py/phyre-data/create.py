...
def get_data(ids, sentence_annotation_file, event_annotation_file):
    events_dict, _ = read_annotation_salient_event_csv([event_annotation_file])
    ...
    for id in tqdm(ids):
        ...
        events_annotation = events_dict[template][task]
        sentences_annotation = sentences_dict[template][task]

        # get src instance
        events = events_annotation["import_labels"]
        timesteps = events_annotation["time_steps"]
        initial_json_file = f"{INITIAL_DATA_DIR}/{template}/{task}.json"
        filtered_json_file = f"{FILTERED_DATA_DIR}/{template}/{task}_list.json"
        src_instance = get_src_instance(initial_json_file, filtered_json_file, events, timesteps)

        # get tgt_summary
        sentence = sentences_annotation[DESCRIPTION_TYPE]
        if sentence == '{}':
            continue
        tgt_summary = get_summary(sentence)
...
def get_src_instance(initial_json_file, filtered_json_file, events, timesteps):
    with codecs.open(initial_json_file, "r", "utf-8") as initial_f, codecs.open(filtered_json_file, "r", "utf-8") as filtered_f:
        ...
        for i, entity in enumerate(entities):
            ...
            for i, record_type in enumerate(init_keys):
                records.append(DELIM.join(create_record(
                    entry[record_type],
                    entity["name"],
                    record_type,
                    "INITIAL_STATE",
        ...
        if DESCRIPTION_TYPE == "simulation_description":
            ...
            if len(records) <= record_len_start:
                print(f"num_significant=0: {initial_json_file}")
            if len([v for k, v in count_significant.items() if v != 0]) != sum(events[:len(steps)]):
...
if __name__ == "__main__":
    ...
    src_instances, tgt_summaries = get_data(ids, SENTENCE_ANNOTATION_FILE, EVENT_ANNOTATION_FILE)
