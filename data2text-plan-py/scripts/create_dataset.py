...
DIR_NAME = "boxscore-data/raw"
if not os.path.exists(f"{DIR_NAME}/inter"):
    os.mkdir(f"{DIR_NAME}/inter")
JSON = f"{DIR_NAME}/train.json"  # input train.json file
ORACLE_IE_OUTPUT = f'{DIR_NAME}/roto_train-beam5_gens.h5-tuples.txt'  # oracle content plan obtained from IE tool
INTER_CONTENT_PLAN = f'{DIR_NAME}/inter/train_content_plan.txt'  # intermediate content plan input to second stage
SRC_FILE = f'{DIR_NAME}/src_train.txt'  # src file input to first stage
TRAIN_TGT_FILE = f"{DIR_NAME}/tgt_train.txt"  # tgt file of second stage
CONTENT_PLAN_OUT = f'{DIR_NAME}/train_content_plan.txt'  # content plan output of first stage
