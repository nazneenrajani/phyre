...
def train_model(model, fields, optim, data_type, model_opt):
    ...
    for epoch in range(opt.start_epoch, opt.epochs + 1):
        ...
        if epoch >= opt.start_checkpoint_at:
            trainer.drop_checkpoint(model_opt, epoch, fields, train_stats)
...
def load_fields(dataset, data_type, checkpoint):
    if checkpoint is not None:
        if opt.override_vocab_path:
            print('Loading vocab from %s.' % opt.override_vocab_path)
            vocab = torch.load(opt.override_vocab_path)
...
def build_optim(model, checkpoint):
    ...
    if opt.train_from and opt.load_checkpoint_optim == 1:
...
def main():
    ...
    if opt.train_from:
        ...
        model_opt.data = opt.data
        model_opt.save_model = opt.save_model
        model_opt.brnn = opt.brnn
        model_opt.encoder_type = opt.encoder_type
        print(model_opt)
        ...
        for key, value in checkpoint["generator"].items():
            if "linear.bias" in key or "linear.weight" in key:
                continue
            new_checkpoint[key] = value
        checkpoint["generator"] = new_checkpoint
