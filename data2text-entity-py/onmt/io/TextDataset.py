    ...
    def read_text_file(path, truncate, side):
        ...
        with codecs.open(path, "r", "utf-8") as corpus_file:
            for i, line in enumerate(corpus_file):
                ...
                if side == "src":
                    # old_entities_len = [22] * 26
                    # old_entities_len.extend([15, 15])
                    # old_entities_list = []
                    # for entities_index in range(0, 28):
                    #     if entities_index < 26:
                    #         old_entities_list.extend([entities_index] * 22)
                    #     else:
                    #         old_entities_list.extend([entities_index] * 15)
                    # old_count_entities = 28
                    ...
                    for feat in feats[0]:
                        if last_feat != feat:
                            feat_idx += 1
                            entities_len.append(feat_len)
                            feat_len = 0
                        entities_list.append(feat_idx)
                        feat_len += 1
                        last_feat = feat
                    entities_len.append(feat_len)
                    count_entities = len(set(feats[0]))
                    # import pdb; pdb.set_trace()
